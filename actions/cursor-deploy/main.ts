...
export async function cursorDeploy({
  ...
  if (deployMode === "default") {
    ...
    if (rollbackFileExists) {
      throw new Error(
        `${branchName} is currently blocked due to an active rollback.`,
      )
    }
  }
  ...
  core.info(
    `Tree hash ${treeHash} is now the active deployment for ${branchName}.`,
  )
 ...
 * @returns treeHash
 */
async function getDeploymentHash(
  ...
  if (deployMode === "rollback") {
    if (!!rollbackCommitHash && !isHeadAncestor(rollbackCommitHash)) {
      throw new Error(
        "The selected rollback commit is not present on the branch",
      )
    }
...
async function isHeadAncestor(commitHash: string) {
  return execIsSuccessful("git merge-base", [
    `--is-ancestor`,
    commitHash,
    `HEAD`,
  ])
